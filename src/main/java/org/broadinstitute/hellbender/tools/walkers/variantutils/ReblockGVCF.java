...
public final class ReblockGVCF extends MultiVariantWalker {
                ...
                if (!genotype.hasPL()) {
                    if (genotype.hasGQ()) {
                        ...
                        final String message = "Homozygous reference genotypes must contain GQ or PL. Both are missing for hom ref genotype at "
                                + originalVC.getContig() + ":" + originalVC.getStart() + " for sample " + genotype.getSampleName() + ".";
    ...
    protected GenotypeBuilder changeCallToHomRefVersusNonRef(final VariantContext lowQualVariant, final Map<String, Object> attrMap) {
        ...
        //for hom-ref variants, drop other ALTs and subset PLs, GQ is recalculated (may be > 0)
            ...
            if (posteriorsKey != null && genotype.hasExtendedAttribute(posteriorsKey)) {
                ...
                final List<Allele> bestAlleles = AlleleSubsettingUtils.calculateMostLikelyAlleles(lowQualVariant, PLOIDY_TWO, 1);
     ...
     */
    private static void composeUpdatedAnnotations(final VariantAnnotatorEngine annotationEngine, final boolean doQualApprox,
                                                  final String posteriorsKey, final VariantContext variant, final Map<String, Object> attrMap,
        ...
        if (annotationEngine.getInfoAnnotations().stream()
            ...
            for (final Allele alt : updatedAllelesVC.getAlleles()) {
                ...
                final GenotypesContext gc = AlleleSubsettingUtils.subsetAlleles(updatedAllelesVC.getGenotypes(),
                        ...
                        GenotypeAssignmentMethod.USE_PLS_TO_ASSIGN, 0, false);  //don't need depth to get PLs for quals
     ...
     */
    private static void copyInfoAnnotations(final VariantAnnotatorEngine annotationEngine,
                                            final List<String> infoFieldAnnotationKeyNamesToRemove,
                                            final VariantContext originalVC, final Map<String, Object> newVCAttrMap,
    ...
    private static void updateMQAnnotations(final VariantContext originalVC, final Map<String, Object> newVCAttrMap) {
        ...
        if (!originalVC.hasAttribute(GATKVCFConstants.RAW_MAPPING_QUALITY_WITH_DEPTH_KEY)) {
            ...
            final int rawMqValue = originalVC.hasAttribute(GATKVCFConstants.RAW_RMS_MAPPING_QUALITY_DEPRECATED) ?
